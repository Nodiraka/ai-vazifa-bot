"""AI Vazifa Bot - FIXED VERSION - Barcha muammolar hal qilindi"""
...
async def presentation_package_selected(update: Update, context) -> int:
    ...
    if row:
        keyboard.append(row)
    keyboard.append([InlineKeyboardButton(t("btn_cancel", lang), callback_data="cancel")])
...
async def presentation_webapp_data(update: Update, context) -> int:
    ...
    try:
        # WebApp data ni olish
        web_app_data = update.effective_message.web_app_data.data
        logger.info(f"WebApp data received: {web_app_data}")
        
        data = json.loads(web_app_data)
        category = data.get("category", "")
        template_id = data.get("id", 1)
        
        logger.info(f"Parsed: category={category}, template_id={template_id}")
...
async def presentation_summary_choice(update: Update, context) -> int:
    ...
    if data == "plan_manual":
        ...
        await start_presentation_creation_query(query, context)
        return PRES_CONFIRM
...
# ========== PRES: CREATE ==========
async def start_presentation_creation_query(query, context):
    ...
    await do_presentation_creation(status_msg, context, user_id, lang,
                                   topic, author, pres_lang, slides_count,
                                   package_key, template_category, template_id,
                                   has_ai_images, plan_mode, plan_1, plan_2, plan_3)
...
async def do_presentation_creation(status_msg, context, user_id, lang,
                                   topic, author, pres_lang, slides_count,
                                   package_key, template_category, template_id,
                                   has_ai_images, plan_mode, plan_1, plan_2, plan_3):
    """Taqdimot yaratish jarayoni"""
    progress_steps = {
    ...
    try:
        ...
        logger.error(f"Taqdimot yaratishda xatolik: {e}", exc_info=True)
        await status_msg.edit_text(
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            LANG_SELECT: [
                CallbackQueryHandler(start_command, pattern="^check_sub$"),
            ],
            MAIN_MENU: [
            ...
            PRES_LANGUAGE: [
                CallbackQueryHandler(presentation_language_selected),
            ],
            PRES_PACKAGE: [
                CallbackQueryHandler(presentation_package_selected),
            ],
            PRES_SLIDES: [
                CallbackQueryHandler(presentation_slides_selected),
            ],
            PRES_TEMPLATE: [
            ...
            PRES_SUMMARY: [
                CallbackQueryHandler(presentation_summary_choice),
            ],
            PRES_PLAN_1: [
    ...
    logger.info("✅ TUZATILGAN BOT ISHGA TUSHDI!")
